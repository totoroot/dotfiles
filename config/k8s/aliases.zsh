...
alias susflux='kubectl get kustomizations,helmreleases,gitrepositories,buckets,helmcharts,helmrepositories,ocirepositories,alerts,providers,receivers -A -o json | jq -r '\'' .items[] | select(.spec.suspend == true) | [.kind,.metadata.namespace,.metadata.name] | @tsv'\'''
